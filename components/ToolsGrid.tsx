...
export default function ToolsGrid() {
  ...
  return (
    <section 
      ...
      className="w-full bg-[#FAF9F6] font-sans relative border-y border-black/10 py-16 md:py-24 overflow-hidden"
    >
            ...
            {[0, 1, 2].map((rowIndex) => (
              <div key={rowIndex} className="grid grid-cols-5 gap-6">
                {TOOLS.slice(rowIndex * 5, (rowIndex + 1) * 5).map((tool) => (
                  <div 
                    key={tool.id} 
                    className="relative h-[140px] bg-black/[0.03] hover:bg-black/10 transition-all duration-300 border border-black/10 hover:border-black/20 rounded-2xl overflow-hidden cursor-pointer flex flex-col items-center justify-center p-4 group"
                  >
                    <div className="w-14 h-14 mb-2 group-hover:-translate-y-2 transition-transform duration-300 relative">
                      <img 
                        src={tool.logo} 
                        alt={tool.name}
                        className={`object-contain w-full h-full ${tool.isDark ? '' : ''}`}
                      />
                    </div>
